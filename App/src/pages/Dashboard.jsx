...
// Note: For this to work in a web environment, GSAP and DrawSVGPlugin must be loaded via script tags.
...
function Counter({ from, to, label }) {
    ...
    return <h3 ref={nodeRef} className="text-2xl font-semibold text-white" />;
}

const MetricCard = ({ title, value, change, isPositive, label = "" }) => (
    <div className="bg-slate-800/70 p-5 rounded-lg backdrop-blur-sm border border-slate-700/80 h-full">
        <p className="text-sm text-slate-400 mb-2">{title}</p>
        <div className="flex items-baseline gap-2">
            {typeof value === 'number' ? <Counter from={0} to={value} label={label} /> : <h3 className="text-2xl font-semibold text-white">{value}</h3>}
        </div>
...
const InfoCard = ({ title, description, children, className }) => (
     <div className={`bg-slate-800/70 p-5 rounded-lg backdrop-blur-sm border border-slate-700/80 flex flex-col ${className}`}>
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-950 to-gray-900 text-white font-sans">
      <div className="flex h-screen">
       <Sidebar/>
        ...
        {/* MAIN CONTENT AREA - FIX: Removed h-screen */}
        <div className="flex-1 flex flex-col">
            ...
            <main className="flex-1 p-8 overflow-y-auto">
                <div className="space-y-6">
                    ...
                    {/* TOP METRIC CARDS */}
                    <motion.section variants={listContainerVariants} initial="hidden" animate="visible" className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-6">
                        ...
                        <motion.div {...cardAnimation} className="h-full"><InfoCard title="Meeting Breakdown" description="Distribution of different meeting types." className="h-full">
                           <div className="relative w-full h-full flex items-center justify-center">
                               <ResponsiveContainer width="100%" height={250}>
                                   <PieChart>
                                       <Pie data={donutData} dataKey="value" nameKey="name" cx="50%" cy="50%" innerRadius={70} outerRadius={100} fill="#8884d8" paddingAngle={5}>{donutData.map((entry, index) => <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />)}</Pie>
                            ...
                            <div className="flex-grow flex items-center justify-center">
                                <ResponsiveContainer width="100%" height={200}>
                                    <RadialBarChart cx="50%" cy="50%" innerRadius="70%" outerRadius="100%" barSize={20} data={storageData} startAngle={90} endAngle={-270}>
                                        ...
                                        <text x="50%" y="50%" textAnchor="middle" dominantBaseline="middle" className="text-3xl font-bold fill-white">{storageData[0].value}%</text>
